...
public class horror_mutant : MonoBehaviour
{
    NavMeshAgent nav;
    GameObject target;
    Animator animator;
    public float stoppingDistance = 20f; // 멈추는 거리
    private float getPlayerDistance = 12f; // 플레이어를 잡는 거리
    private float slowspeed = 0.5f; // 느리게 적용할 속도값
    ...
    void Update()
    {
        ...
        if (distanceToTarget < stoppingDistance)
        {
            if (!isSpeedModified)
            {
                ...
                StartCoroutine(RestoreNavSpeed(4.5f)); // 원래 속도로 복귀하는 함수 5초뒤 호출
            }
            ...
            if (distanceToTarget < getPlayerDistance && !hasPlayedAudio)
            {
                ...
                // 플레이어의 카메라를 바라보도록 회전
                Vector3 targetPosition = new Vector3(nav.transform.position.x, nav.transform.position.y + 4, nav.transform.position.z);
